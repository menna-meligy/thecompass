...
CREATE POLICY IF NOT EXISTS "Users can upload receipts" ON storage.objects
  FOR INSERT WITH CHECK (
    bucket_id = 'receipts' AND auth.role() = 'authenticated'
  );

-- Allow users to view their own receipts
CREATE POLICY IF NOT EXISTS "Users can view their own receipts in storage" ON storage.objects
  FOR SELECT USING (
    bucket_id = 'receipts' AND auth.uid()::text = owner
  );
